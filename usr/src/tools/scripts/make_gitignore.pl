...
for (my $j = 0; $j <= 4 && $j < @dirs; ++$j) {
	...
	my @addenda = map { s`^#\Q$dotdot`/`r }
	  grep { m`^#\Q$dotdot\E[^\.]` } @lines;
	if (@addenda > 0) {
		my $addendum = join("", map { s/$/\n/r } @addenda);
	...
	# if -d is used, that makes regular, non-discriminated .gitignore-<subdir>
	# files ineligible, so clear any that exist
	if ($opt_d) {
...
sub normalize_gitignore_entry {
	...
	if ($entry =~ m`/.`) {
		if (-l $entry) {
			...
			return "#$entry" if $realentry !~ m`^\Q$cwd/`;
			$entry = $realentry;
			$entry =~ s/^\Q$cwd//;
		}
	}
	...
	# is confused)
	$entry .= "/" if $entry !~ m`/$` && -d $entry && !-l $entry;
